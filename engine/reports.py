...
def render_html(report):
    ...
    rows=''.join('<tr>'+''.join('<td>'+e(f.get(k,''))+'</td>' for k in ('severity','tool','title','url','confidence','evidence'))+'</tr>' for f in sorted(report['findings'],key=lambda f:SEVERITIES.index(f.get('severity','info'))))
    ...
    return ("<!doctype html><html lang='ru'><meta charset='utf-8'><meta name='viewport' content='width=device-width'><title>XYRO · Отчёт</title>"
      ...
      '<h2>Находки</h2><table><tr><th>Уровень<th>Модуль<th>Находка<th>URL<th>Доказательство<th>Данные</tr>'+rows+'</table>'
...
def csv_report(report):
    ...
    columns=('severity','tool','title','url','confidence','template_id','evidence','remediation')
...
def sarif_report(report):
    ...
    for item in report['findings']:
        ...
        results.append({'ruleId':rule,'level':'error' if item['severity'] in ('critical','high') else 'warning' if item['severity']=='medium' else 'note',
                        ...
                        'properties':{'severity':item['severity'],'confidence':item['confidence'],'tool':item['tool']}})
